<script lang="ts">
  import thumbnail1 from "$lib/images/blurry-gradient-haikei.png";
  ...
  //     body: bio,
  //     image: thumbnail,
  //     modalClasses: "!bg-gradient-to-tr !from-primary-200 !to-tertiary-200",
  //     meta: {worksheetsURL: worksheetsURL}
  // };
  ...
  function TriggerModal() {
    ...
    if (
      $page.url.pathname == "/content/videos" ||
      $page.url.pathname == "/content/videos/"
    ) {
      modalStore.trigger(videoModal);
    }
    if (
      $page.url.pathname == "/content/articles" ||
      $page.url.pathname == "/content/articles/"
    ) {
...
{#if $currentTopic === ""}
  <button
    class="card card-hover overflow-hidden bg-white"
